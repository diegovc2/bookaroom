						...
						if( is_array( $roomContList['id'][$rc_val]['rooms'] ) ) {
							foreach( $roomContList['id'][$rc_val]['rooms'] as $room_key => $room_val ) {
								...
								if( count( $roomList['id'][$room_val]['amenity'] ) !== 0 ) {
									...
									$amenityListDisp = implode( ', ', $a_list );
								}
								?>
				<tr>
					<td colspan="2" class="bufferLeft"><?php echo $roomList['id'][$room_val]['desc']; ?></td>
				</tr>
				<tr>
					<td colspan="2" class="bufferLeftDouble"><em><?php _e( 'Amenities', 'book-a-room' ); ?>: <?php echo $amenityListDisp; ?></em>
					</td>
				</tr>
				<?php 
							}
